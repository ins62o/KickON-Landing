...
const supportEmail = "kickon.offical@gmail.com";
...
const sections: LegalSection[] = [
  ...
  },
  {
    ...
    content: (
      <>
        <p>앱을 삭제했거나 로그인할 수 없다면 KICKON 개발자 연락처인 <a href={`mailto:${supportEmail}?subject=${emailSubject}`}>{supportEmail}</a>로 아래 정보를 보내 주세요.</p>
        <ul>
  ...
  },
  {
    ...
    content: (
      <>
        ...
        <p>7일이 지나면 이 해시도 삭제합니다. 관계 법령에 따라 별도 보관이 필요한 정보가 생기면 해당 항목과 기간을 <a href="/privacy/">개인정보 처리방침</a>에 공개합니다.</p>
      </>
    ),
  },
  {
  ...
  },
  {
    id: "social-account",
    title: "소셜 계정과의 관계",
    content: <p>KICKON 계정을 삭제해도 Apple 계정이나 Kakao 계정 자체는 삭제되지 않습니다. 소셜 로그인 연결 상태는 각 서비스의 계정 설정에서도 별도로 확인하고 해제할 수 있습니다.</p>,
  },
];
...
export function AccountDeletionPage() {
  return (
    <LegalPage
      ...
      sections={sections}
      notice={<><strong>삭제 전 확인:</strong> 계정 삭제는 취소할 수 없으며, 게시글과 직관 기록을 포함한 활동 데이터도 복구할 수 없습니다.</>}
    />
  );
}
